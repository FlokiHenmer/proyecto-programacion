...
export default function GerenteMenu({ anchorEl, onClose, onChangeEstado, onBorrar }) {
  return (
    <Menu 
      anchorEl={anchorEl} 
    ...
    >
      <MenuItem onClick={() => onChangeEstado("Activo")}>
        <ListItemIcon><PersonIcon fontSize="small" sx={{ color: GREEN_TEXT }} /></ListItemIcon>
      ...
      <MenuItem onClick={() => onChangeEstado("Inactivo")}>
        <ListItemIcon><PersonOffIcon fontSize="small" sx={{ color: YELLOW_TEXT }} /></ListItemIcon>
      ...
      <MenuItem onClick={onBorrar} sx={{ color: RED, "&:hover": { bgcolor: RED_BG } }}>
        ...
        <ListItemText sx={{ "& .MuiTypography-root": { fontSize: 13, fontWeight: 600 } }}>Eliminar Gerente</ListItemText>
      </MenuItem>
    </Menu>
  );
}